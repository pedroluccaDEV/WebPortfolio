  ...
  <div class="header">
    ...
    <div class="links">
      ...
      <button class="toggle" @click="toggleMenu">
        <i v-show="!isMenuVisible" class="fas fa-bars" style="color: #fff; margin: auto;"></i>
        <i v-show="isMenuVisible" class="fas fa-times" style="color: #fff; margin: auto;"></i>
      </button>
      <AppMenu v-if="shouldShowMenu" :isVisible="isMenuVisible" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    shouldShowMenu() {
      return this.$q.screen.width !== 'xs' && this.$q.screen.width !== 'sm';
    },
  },
  mounted() {
